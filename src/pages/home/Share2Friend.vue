<template>
  <div class="Share2Friend">
    ...
    <div ref="list" class="content" @scroll="scroll">
      <div class="list">
        <div v-for="(value, name)  of friendsOk">
          <div :class="name === '#'?'top':name" class="row no-active no-padding index">
            <span>{{ name }}</span>
          </div>
          <div class="local-row" v-for="item  of value" @click="item.select = !item.select">
  ...
  methods: {
    async getFriends() {
      ...
      if (res.code === this.SUCCESS) {
        ...
        res.data.map(v => {
          if (this.friendsOk[v.pinyin]) {
            this.friendsOk[v.pinyin].push(v)
          } else {
            this.friendsOk[v.pinyin] = [v]
          }
        })
        console.log(this.friendsOk)
        if (this.friendsOk['#']) {
