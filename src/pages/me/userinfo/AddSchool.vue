  ...
  <div class="school">
    ...
    <div class="content">
      ...
      <div
        class="row"
        @click="
          $nav('/me/display-type', { displayType: localSchool.displayType })
        "
      >
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      displayType() {
        if (this.localSchool.displayType === enums.DISPLAY_TYPE.ALL)
          return '公开可见'
        if (this.localSchool.displayType === enums.DISPLAY_TYPE.SCHOOL)
          return '校友可见'
        if (this.localSchool.displayType === enums.DISPLAY_TYPE.ME)
          return '仅自己可见'
      },
      school() {
    ...
    methods: {
      showJoinTimeDialog() {
        new MobileSelect({
          ...
          wheels: [
            {
              data: Array.apply(null, { length: 50 }).map(
